    ...
    <table class="w-full text-sm text-left">
        <thead class="flex flex-col w-full text-white bg-black rounded-t-md">
            <tr class="flex w-full mx-2">
                <th class="w-6/12">Store</th>
                <th class="w-4/12">Fechas Previstas</th>
                <th class="w-2/12 mr-4">Estado</th>
            </tr>
        </thead>
        <tbody class="flex flex-col w-full overflow-y-scroll bg-grey-light" style="height: 55vh;">
            @foreach ($camptiendas as $camptienda)
            <tr class="flex w-full py-1 {{ $loop->even ? 'bg-blue-50' : 'bg-blue-100'}} hover:bg-gray-100 hover:cursor-pointer" onclick="location.href = '{{ route('montador.edittienda',$camptienda) }}'" color="gray" >
                <td class="w-6/12 ml-2">{{$camptienda->store_id}} {{$camptienda->tienda->name}} {{$camptienda->tienda->city !='' ? '('.$camptienda->tienda->city.')' : ''}}</td>
                <td class="flex-col w-4/12">
                ...
                <td class="w-2/12">{{$camptienda->estadomonta}} </td>
            </tr>
            @endforeach
        </tbody>
    </table>
