    ...
    <div class="w-5"><x-icon.campground-a href="{{route('campaign.index') }}" class="w-5 text-orange-500" title="Campañas"/></div>
    @endcan
    @endif
    @can('campaign.edit')
    <div class="w-5">
        <x-icon.filter-a href="{{route('campaign.filtrar', $campaign) }}" title="Filtrar" class="w-5 mr-1 text-black transform hover:text-black hover:scale-125"/></a>
    </div>
    @endcan
    @can('campaign.index')
    <div class="w-5">
