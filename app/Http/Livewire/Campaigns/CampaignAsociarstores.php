...
class CampaignAsociarstores extends Component
{
    ...
    public function asocia($disponible){
        $existe=CampaignStore::where('campaign_id',$this->campaignid)->where('store_id',$disponible['ident'])->first();
        if(!$existe)
            CampaignStore::create([
                'campaign_id'=>$this->campaignid,
                'store_id'=>$disponible['ident'],
    ...
    public function getRowsdispQueryProperty(){
        ...
        ->select('store_id as ident','name as name')
